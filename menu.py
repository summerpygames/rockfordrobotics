...
class MovieMenu(AnyMenu):
    ...
    """Class for the movie that plays before a stage starts
    
    This class is given the endgame, that is basically a tuple of all that would
    normally be passed into the level if it was executing right off the menu,
    but in this case we show a couple pictures before the game starts.

    The pictures are part of a database file, and therfor part of gameplay, so
    calling upon the gameplay object that is passed in is good enough. this
    thing should be passed in 
...
#                             trigger = None)          What to launch on click
        ...
        locks = self.gp.get_unlock_stage(self.grade)

        self.deep = Button(   sel_svg = stage_deep_sel if locks[1] else stage_locked_sel,
                              des_svg = stage_deep_des if locks[1] else stage_locked_des,
                              size = ( 300, None ),
                              callout = 'deep',
                              initsel = True,
